...
#import <QuartzCore/QuartzCore.h>
    ...
    #define SIZE_PAN_FROM_EDGE_MARGIN 80
...
- (void)setupCenterView
{
    ...
    if (!self.centerVC)
    {
        ...
        self.centerVC.view.backgroundColor = [UIColor redColor];
        self.centerVC.view.frame = self.view.bounds;
    }
...
- (void)viewPanned:(UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateChanged)
    {
        ...
        if (self.sidebarVC.view.center.x + translatedPoint.x <= HalfViewPos)
        {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    if (gestureRecognizer == self.openSidebarPanGesture)
    {
        ...
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone)
        {
            ...
            {
                // Default Preferences view = 268 margin = 80
                Margin = SIZE_PAN_FROM_EDGE_MARGIN;
            }
        ...
        if ((self.direction == UISidebarViewControllerDirectionLeft
                && startingPoint.x <= Margin)
            || (self.direction == UISidebarViewControllerDirectionRight
                && startingPoint.x >= CGRectGetWidth(self.view.bounds) - Margin)) {
            return true;
        }
        return false;
    }
