...
- (void) watcherThread:(id)sender
{
    int					n;
    struct kevent		ev;
    struct timespec     timeout = { 0, 0 };     // 1 second timeout. Should be longer, but we need this thread to exit when a kqueue is dealloced, so 1 second timeout is quite a while to wait.
    ...
    while(_keepWatcherThreadRunning)
    {
        @try 
        {
            ...
            if (n > 0)
            {
                ...
                if (ev.filter == EVFILT_VNODE)//EVFILT_VNODE)
                {
